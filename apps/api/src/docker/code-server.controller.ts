...
@Controller("workspaces/:workspaceId/envs/:envId/code-server")
@UseGuards(JwtAuthGuard)
export class CodeServerController {
  constructor(
  ...
  async action(
    ...
    if (action === "stop") {
      await this.codeServer.stop(envId);
      return { ok: true };
    }

    const result = await this.codeServer.start(envId);
    if (!result.ok) {
      throw new HttpException(result.error, 409);
    }
    // Same-origin proxied path when the api is containerized (Traefik routes
    // /api/code-server/view/ → api → reverse proxy → the container); the
    // direct loopback URL on a dev host, where it already worked. The old
    // hardcoded http://127.0.0.1:<port> was the *browser's* loopback, which
    // is why "open in browser" failed on remote deploys.
    const url = await this.codeServer.viewerUrl(envId);
